...
void generate_keys(privateKey* ku, publicKey* kp)
{
...
void block_encrypt(mpz_t C, mpz_t M, publicKey kp)
{
	/* C = M^e mod n */
...
int encrypt(char cipher[], char message[], int length, publicKey kp)
{
...
void block_decrypt(mpz_t M, mpz_t C, privateKey ku)
{
	mpz_powm(M, C, ku.d, ku.n);
	return;
}

int decrypt(char* message, char* cipher, int length, privateKey ku)
{
	int msg_idx = 0;
	char buf[BLOCK_SIZE];
	*(long long*)buf = 0ll;
